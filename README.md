...
4. **⭐ 사이드바 관심기업 Quick Add**: 어디서든 티커/한글명으로 즐겨찾기 추가 (DB 검증 + yfinance 실존 확인)
5. **🔔 인앱 알림 시스템**: 관심 기업 주가 급등락(±3%/5%/10%) 및 뉴스 감성 분석 기반 자동 알림 (사용자별 임계값 설정)
6. **🔍 Text-to-SQL**: 자연어 질의를 통한 복잡한 재무 재표 검색
7. **🕸️ Neo4j GraphRAG**: 기업 관계망을 그래프 DB로 관리, Cypher 쿼리 기반 분석
8. **📰 FinBERT 감성 분석 파이프라인**: GraphRAG와 연계한 심층 감성 분석 (Financial PhraseBank 97% 정확도 검증, `NewsAnalyzerService`)
...
### Phase 7: In-App Notification System ✅ 완료

- [x] **Notification DB 모델**: `Notification` 모델 (ticker, title, message, type, is_read) 및 `Watchlist.alert_threshold_percent` 필드 추가
- [x] **스케줄러 연동**: 매일 05:00 S&P 500 주가 수집 → 사용자별 등락률 임계값 초과 시 자동 알림 생성
- [x] **FinBERT 뉴스 감성 알림**: 매일 06:00 뉴스 감성 분석 → 강한 긍정/부정 뉴스 알림 생성
- [x] **알림 API**: `GET /api/notifications/`, `POST /api/notifications/<id>/read/` 엔드포인트 구현
- [x] **프론트엔드 UI**: 네비게이션 바 🔔 아이콘 (빨간 뱃지 + 드롭다운), 사이드바 임계값 설정 (±3%/±5%/±10%)
- [x] **입력 검증 강화**: 관심기업 추가 시 `isascii()` + `yfinance history()` 이중 검증으로 유효하지 않은 티커/한글 입력 원천 차단
- [x] **실적 캘린더 매핑 수정**: 분기별 실적 발표일(Reporting Date) 기준으로 조회되도록 수정
...
│   ├── sql/                  # SQL Modules
│   ├── tools/                # Agent Tools (Exchange, Favorites, Scheduler Manager)
│   └── utils/                # Utilities (Charts, PDF, Ticker Resolver, Ticker Search Agent)
├── scripts/                  # ETL & Batch Scripts (sp500_scheduler.py 포함)
